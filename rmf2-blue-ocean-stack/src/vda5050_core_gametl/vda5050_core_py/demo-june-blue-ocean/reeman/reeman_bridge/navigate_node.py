#!/usr/bin/env python3
"""POST /cmd/nav — GameTL navigate demo (spellbook navigate_reeman)."""
...
from navigate import monitor_reeman_move_after_dispatch, navigate


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Dispatch a standard move to X Y [ORI] via Reeman POST /cmd/nav.",
    )
    parser.add_argument(
        "rest",
        nargs="*",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="target_ori radians CCW from East (optional)",
    )
    parser.add_argument(
        "--target-accuracy",
        type=float,
    ...
    parser.add_argument(
        "--no-monitor",
        action="store_true",
        help="POST only; skip nav_status watch",
    )
