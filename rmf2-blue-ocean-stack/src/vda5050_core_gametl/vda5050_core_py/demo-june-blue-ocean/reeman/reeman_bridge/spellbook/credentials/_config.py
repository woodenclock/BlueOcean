...
def timeout_seconds(cfg: RobotConfig | None = None) -> float:
    """Active robot timeout in seconds (for ``requests``)."""
    if cfg is None:
        cfg = robot_config()
...
def _robot_ip_from_env() -> str | None:
    """``REEMAN_ROBOT_IP`` (host or host:port) or ``REEMAN_BASE_URL`` (http://host)."""
    raw = os.environ.get("REEMAN_ROBOT_IP", "").strip()
    if raw:
        return raw
    base = os.environ.get("REEMAN_BASE_URL", "").strip()
    if not base:
        return None
...
def robot_config(data: dict[str, Any] | None = None) -> RobotConfig:
    ...
    robot_ip = _robot_ip_from_env() or str(entry["robot_ip"])
    return RobotConfig(
        PREFIX=prefix,
...
class _ActiveRobot:
    """Module-level active-robot proxy used by every spellbook command as
    ``from credentials import CONSTANTS as ROBOT``.

    ``PREFIX`` / ``ROBOT_NAME`` / ``TIMEOUT_MS`` come from ``CONSTANTS.yml`` (read
    once). ``ROBOT_IP`` is resolved LIVE on every access — ``REEMAN_ROBOT_IP`` /
    ``REEMAN_BASE_URL`` env wins over the yaml entry.

    This matters because adapter clients import this module at startup (freezing
    everything) but only export their per-robot endpoint (from the VDA5050
    master /robots) later, in ``main()``. A value frozen at import time pinned
    every adapter to the same ``CONSTANTS.yml`` robot, so two Reeman adapters
    (reeman-1, reeman-2-blue) both polled the same robot. Reading ROBOT_IP from
    the env on access lets each process honour its own runtime-exported endpoint.
    """
    ...
    def ROBOT_IP(self) -> str:
        return _robot_ip_from_env() or self._cfg.ROBOT_IP


CONSTANTS = _ActiveRobot()
